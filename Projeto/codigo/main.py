...
def preparar_simulacao(num_navios=50, politica="passiva", seed=None):
...
def simulacao_estatica(num_navios=50, politica="passiva", seed=None):
    ambiente, vant = preparar_simulacao(num_navios, politica, seed)
    ambiente.plotar_cenario(vant=vant, save=True)

def simulacao_animada(num_navios=50, politica="passiva", seed=None):
    ambiente, vant = preparar_simulacao(num_navios, politica, seed)
    filename = vant.politica + ".gif"
    ambiente.animar_cenario(vant, filename=filename)

def simulacao_texto(num_navios=50, politica="passiva", seed=None):
    ambiente, vant = preparar_simulacao(num_navios, politica, seed)
